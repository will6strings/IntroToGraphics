""" Program: DragonSlayer.py
    ...
    Unless they have the golden sword giving them ultimate power.
    """
...
''' 1.1 edit: Changed the multiple story functions (theCave, theRiver, thePath, theCabin)
    ...
    Also fixed a major spelling error firey instead of the correct fiery'''
...
'''3.0 edit: Added functionality to the golden sword. Now if the player finds the
   sword they play the game again with the sword. Having the golden sword automatically
   ensures victory against the dragon. This is done through passing boolean paramaters
   stating wether they have the sword or not. They can also lose the sword by running
   into bandits.'''
...
"""Function:darkForest
   ...
   Input: A boolean stating if the sword was found or not
   Output:A boolean stating if the player found the golden sword
   """
def darkForest(swordFound):
    ...
    if choice == "1":
        ...
        if choice == "1":
            ...
            print("Congradulations you found the ultimate weapon.")
            print("You may now start your adventure again with the")
            print("GOLDEN SWORD")
            #lets the game know the sword was found
            swordFound = True
            time.sleep(3)
    #they choose the light
        ...
        if choice == "1":
            ...
            #If player has the golden sword they loose it
            if swordFound == True:
                print("You now must continue on without your")
                print("Golden Sword until you find it again.")
                swordFound = False
                time.sleep(2)
...
def fieryMountain(swordFound):
    ...
    if choice == "1":
        ...
        if choice == "1":
            ...
            if str(choice) == "1" or swordFound == True:
...
"""Function:Main
   ...
   """
def main():
    #variable used to determine if the player found the golden sword
    swordFound = False
    #variable stating the player has found the sword
    goldenSword = False
    ...
    while play:
        ...
        if choice == "1":
            ...
            fieryMountain(goldenSword)    
        #go to forest
        elif choice == "2":
            print("You walk towards the forest")
            #Checks if the golden sword was found and plays the darkForest story
            swordFound = darkForest(goldenSword)
            #adds the golden sword to the player
            goldenSword = swordFound
